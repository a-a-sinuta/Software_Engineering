...
``def __init__(self, index):`` - магический метод для инициализации нового объекта класса, который запускается при создании нового объекта и принимает параметры: self(ссылка на объект(этот параметр мы напрямую не передаем, за нас это автоматически делает Python)) и index.ps как конструктор в java
